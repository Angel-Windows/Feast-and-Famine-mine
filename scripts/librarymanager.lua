...
if rawget(_G, "TheFrontEnd") then
    ...
    if rawget(_G, "LibraryManager") == nil or
        (rawget(_G, "LibraryManager") ~= nil and rawget(_G, "LibraryManager").VERSION < VERSION) then

        -- uninstall old Library Manager
        if rawget(_G, "LibraryManager") then
            rawget(_G, "LibraryManager").UninstallLibraryManager()
        end
        ...
        local function SetPostiveNumberTable(tbl)
            ...
            return setmetatable(tbl, {
                __index = __index_posnumber,
                __newindex = __newindex_posnumber
            })
        end
        ...
        local function new_UpdateForWorkshop(self, force_refresh)
            ...
            for k, v in pairs(self.optionwidgets_server) do
                if not v.is_downloading and LibraryManager.server_dependency_list[v.mod.modname] > 0 and
                    not KnownModIndex:IsModEnabled(v.mod.modname) then
                    local old_ShowModDetails = self.ShowModDetails
                    self.ShowModDetails = function()
                    end
                ...
                if not v.is_downloading and (LibraryManager.disable_list[v.mod.modname] or false) and
                    KnownModIndex:IsModEnabled(v.mod.modname) then
                    local old_ShowModDetails = self.ShowModDetails
                    self.ShowModDetails = function()
                    end
                    local oldModType = self.currentmodtype
            ...
            if LibraryManager.server_dependency_list[modname] > 0 and KnownModIndex:IsModEnabled(modname) then
                ...
                for k, v in pairs(LibraryManager.dependency_list) do
                    for i1, v1 in ipairs(v) do
                        if v1 == modname then
                            table.insert(modDependentList, k)
                        end
                    end
                end
                ...
                TheFrontEnd:PushScreen(TextListPopup(BuildModList(modDependentList),
                    STRINGS.UI.MODSSCREEN.MOD_DEPENDENTS_TITLE, string.format(STRINGS.UI.MODSSCREEN.MOD_HAS_DEPENDENTS,
        ...
        local function new_Create(self, warnedOffline, warnedDisabledMods, warnedOutOfDateMods)
            for k, v in pairs(LibraryManager.server_dependency_list._) do
                if v > 0 and not KnownModIndex:IsModEnabled(k) then
                    TheFrontEnd:PushScreen(PopupDialogScreen(STRINGS.UI.MODSSCREEN.REQUIRED_MODS_DOWNLOADING_TITLE,
        ...
        local function AutoSubscribeAndEnableWorkshopMods(modsList)
            ...
            local function QueryName(modname, modtable, modnumber)
                if IsWorkshopMod(modname) then
                    TheSim:QueryServer("http://steamcommunity.com/sharedfiles/filedetails/?id=" ..
                                           GetWorkshopIdNumber(modname), function(result, isSuccessful, resultCode)
            ...
            local function ConfirmModDependenciesDownload()
                ...
                TheFrontEnd:PushScreen(TextListPopup(BuildModListQueryNames(modsList),
                    STRINGS.UI.MODSSCREEN.MOD_DEPENDENCIES_TITLE, string.format(
                        STRINGS.UI.MODSSCREEN.MOD_HAS_DEPENDENCIES, modenv.modinfo.name), {
                        {
