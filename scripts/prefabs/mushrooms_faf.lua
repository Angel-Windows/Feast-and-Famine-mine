local assets =
{
    Asset("ANIM", "anim/mushroom_tree_chunks.zip"),
}

local function redpoison(inst, eater)
    if math.random() <= 0.01 and
        not (eater.components.health ~= nil and eater.components.health:IsDead()) and
        not eater:HasTag("playerghost") and 
        not eater:HasTag("plantkin") then
        if eater:HasTag("player") then 
            eater:PushEvent("death", {cause = "red_chunk_bloom_death"})
        else
...
local function MakeMushroom(data)
    ...
    return 
           Prefab(data.animname.."_chunk", chunkfn, assets, prefabs),
