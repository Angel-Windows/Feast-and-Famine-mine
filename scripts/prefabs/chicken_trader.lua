...
SetSharedLootTable('rooster', {{'drumstick', 1.0}, {'drumstick', .75}, {'strawhat', .05}})
    ...
    return not inst:IsInLimbo() and FindEntity(inst, TUNING.PIG_TARGET_DIST, function(guy)
        return inst.components.combat:CanTarget(guy) and
                   (guy:HasTag("monster") or
                       (guy.components.inventory ~= nil and guy:IsNear(inst, TUNING.BUNNYMAN_SEE_MEAT_DIST) and
                           guy.components.inventory:FindItem(is_egg) ~= nil))
    end, {"_combat", "_health"}, -- see entityreplica.lua
    nil, {"monster", "player"}) or nil
end
...
local function battlecry(combatcmp, target)
    local strtbl =
        target ~= nil and target.components.inventory ~= nil and target.components.inventory:FindItem(is_egg) ~= nil and
            "CHICK_EGG_BATTLECRY" or "CHICK_BATTLECRY"
