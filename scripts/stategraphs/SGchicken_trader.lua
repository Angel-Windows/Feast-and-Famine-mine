...
local actionhandlers = {ActionHandler(ACTIONS.GOHOME, "gohome"), ActionHandler(ACTIONS.PICKUP, "pickup"),
                        ActionHandler(ACTIONS.EAT, "eat"), ActionHandler(ACTIONS.TAKEITEM, "pickup"),
                        ActionHandler(ACTIONS.DROP, "dropitem")}

local events = {CommonHandlers.OnStep(), CommonHandlers.OnLocomote(true, true), CommonHandlers.OnSleep(),
                CommonHandlers.OnFreeze(), EventHandler("attacked", function(inst)
    if not inst.components.health:IsDead() and not inst.sg:HasStateTag("transform") then
        inst.sg:GoToState("hit")
    end
end), EventHandler("death", function(inst)
    inst.sg:GoToState("death")
end), EventHandler("doattack", function(inst)
    if not inst.components.health:IsDead() and not inst.sg:HasStateTag("transform") then
...
local states = {State {
    ...
    timeline = {TimeEvent(20 * FRAMES, function(inst)
...
CommonStates.AddSleepStates(states, {
    ...
    sleeptimeline = {TimeEvent(40 * FRAMES, function(inst)
        inst.SoundEmitter:KillSound("roostersleep")
        inst.SoundEmitter:PlaySound("chickenfamily/chickenfamily/roostersleep", "roostersleep")
    end)}
})
