...
local actionhandlers = {ActionHandler(ACTIONS.EAT, "eat_pre")}

local events = {CommonHandlers.OnSleep(), CommonHandlers.OnFreeze(), EventHandler("attacked", function(inst)
    if inst.components.health:GetPercent() > 0 then
        inst.sg:GoToState("hit")
    end
end), EventHandler("death", function(inst)
    inst.sg:GoToState("death")
end), EventHandler("trapped", function(inst)
    inst.sg:GoToState("trapped")
end), EventHandler("locomote", function(inst)
...
local states = {State {
    ...
    events = {EventHandler("animover", function(inst, data)
        inst.sg:GoToState("idle")
    end)}
}, State {
    name = "eat_pre",
    ...
    timeline = {TimeEvent(5 * FRAMES, function(inst)
        inst.Physics:Stop()
